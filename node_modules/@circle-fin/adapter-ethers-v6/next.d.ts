...
import { Provider, Signer, TransactionReceipt, Eip1193Provider } from 'ethers';
import { Abi } from 'abitype';
import { TransactionInstruction, Signer as Signer$1, AddressLookupTableAccount } from '@solana/web3.js';
 ...
 * ```
 */
interface AdapterContext$1<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> {
 ...
 * ```
 */
interface PrepareResult<TInput, TBuilt, TRaw, TPrepared extends PreparedTransactionBase = PreparedTransaction<TInput, TBuilt, TRaw>> {
    /**
 ...
 */
interface Adapter$1<TContext, TReadInput, TWriteInput, TChain extends ChainDefinition = ChainDefinition> {
    /** The adapter context containing configuration, clients, and capabilities. */
    readonly ctx: TContext;
    /** Execute a read operation. */
    read<T>(input: TReadInput, operation: OperationMeta, invocation?: InvocationMeta): Promise<T>;
    /** Prepare one or more write operations. */
    prepare(input: TWriteInput | TWriteInput[], operation: OperationMeta, invocation?: InvocationMeta): Promise<PrepareResult<unknown, unknown, unknown>>;
...
type AdapterReadInputOf<T> = T extends Adapter$1<unknown, infer R, unknown> ? R : never;
/** Extract the chain type from an adapter. */
type AdapterChainOf<T> = T extends Adapter$1<unknown, unknown, unknown, infer C> ? C : never;
...
/**
 * Type definitions for the createPrimitive factory.
 *
 * @packageDocumentation
 */
 ...
 */
type PrimitiveFunction<TInput, TOutput> = (input: TInput, operation: OperationMeta, invocation?: InvocationMeta) => Promise<TOutput>;
/**
 ...
 * @typeParam TInput - The input type for the primitive.
 */
type GenericPrimitiveFunction<TInput> = <TOutput = unknown>(input: TInput, operation: OperationMeta, invocation?: InvocationMeta) => Promise<TOutput>;
/**
 ...
 * ```
 */
interface ActionOptions extends InvocationMeta {
    /**
 ...
 */
interface ReadAction<TInput, TOutput> {
    /**
     ...
     */
    (input: TInput, operation: OperationMeta, options?: ActionOptions): Promise<TOutput>;
    /** Action name. */
    readonly name: string;
    /**
 ...
 */
type PreparedWriteAction<TOutput = void> = PrepareResult<unknown, unknown, unknown> & {
 ...
 */
interface WriteAction<TInput, TOutput = void> {
    /**
     ...
     */
    (input: TInput, operation: OperationMeta, options?: ActionOptions): Promise<WriteResult>;
    /**
     ...
     */
    prepare(input: TInput, operation: OperationMeta, options?: ActionOptions): Promise<PreparedWriteAction<TOutput>>;
    /** Action name. */
    readonly name: string;
    /**
     * Human-readable description of what the action does.
 ...
 *
 * @remarks
 * This is a chain-agnostic definition. The adapter provides the
 * execute function that handles chain-specific logic.
 *
 * Output type is `BalanceOutput` - consistent across all chains.
 *
 * @example
 * ```typescript
 * // EVM binding
 * const boundAction = tokenBalanceOf.bind({
 *   adapter,
 *   execute: async (input, ctx) => {
 *     const balance = await ctx.read<bigint>({
 *       address: input.token,
 *       abi: erc20Abi,
 *       functionName: 'balanceOf',
 *       args: [input.walletAddress ?? ctx.address],
 *     })
 *     return { raw: balance }
 *   },
 * })
 * ```
 */
/** Bound read action type for `token.balanceOf`. */
type TokenBalanceOfAction = ReadAction<TokenBalanceOfInput, BalanceOutput>;
...
/** Bound read action type for `token.allowance`. */
type TokenAllowanceAction = ReadAction<TokenAllowanceInput, AllowanceOutput>;
...
/** Bound write action type for `token.approve`. */
type TokenApproveAction = WriteAction<TokenApproveInput, ApproveOutput>;
...
/** Bound write action type for `token.transfer`. */
type TokenTransferAction = WriteAction<TokenTransferInput>;
...
/** Bound write action type for `token.increaseAllowance`. */
type TokenIncreaseAllowanceAction = WriteAction<TokenIncreaseAllowanceInput>;
...
/** Bound read action type for `native.balanceOf`. */
type NativeBalanceOfAction = ReadAction<NativeBalanceOfInput, BalanceOutput>;
...
/** Bound write action type for `native.transfer`. */
type NativeTransferAction = WriteAction<NativeTransferInput>;
 ...
 */
interface BaseActionRegistry {
    'token.balanceOf': TokenBalanceOfAction;
    'token.allowance': TokenAllowanceAction;
    'token.approve': TokenApproveAction;
    'token.transfer': TokenTransferAction;
    'token.increaseAllowance': TokenIncreaseAllowanceAction;
    'native.balanceOf': NativeBalanceOfAction;
    'native.transfer': NativeTransferAction;
    /**
     * @deprecated Use `token.balanceOf` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.balanceOf': ReadAction<StablecoinBalanceOfInput, BalanceOutput>;
    /**
     * @deprecated Use `token.allowance` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.allowance': ReadAction<StablecoinAllowanceInput, AllowanceOutput>;
    /**
     * @deprecated Use `token.approve` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.approve': WriteAction<StablecoinApproveInput, ApproveOutput>;
    /**
     * @deprecated Use `token.transfer` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.transfer': WriteAction<StablecoinTransferInput>;
    /**
     * @deprecated Use `token.increaseAllowance` with `{ token: 'USDC' }` instead.
     * These USDC convenience aliases will move to `@core/adapter-compat`.
     */
    'usdc.increaseAllowance': WriteAction<StablecoinIncreaseAllowanceInput>;
}
 ...
 */
interface AssembledAdapterShape<TCtx extends AdapterContext$1, TChainType extends string, TRead extends AnyFunction, TPrepare extends AnyFunction, TWait extends AnyFunction, TActions extends AnyRecord> extends AdapterEventSubscription {
 ...
 * ```
 */
declare class EthersNonceManager {
    /**
     ...
     * ```
     */
    allocate(provider: Provider, chainId: number, address: string): Promise<number>;
    /**
     ...
     * ```
     */
    clear(chainId: number, address: string): void;
    /**
     ...
     * ```
     */
    shouldRetryNonceError(error: unknown, userProvidedNonce: boolean): boolean;
    /**
     ...
     * ```
     */
    resyncAndAllocate(provider: Provider, chainId: number, fromAddress: string): Promise<number>;
}

/**
 * Configuration options for creating an Ethers adapter context.
 *
 ...
 *   getSigner: ({ chain }) => new Wallet(privateKey),
 * }
 * ```
 */
interface AdapterContextOptions<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> {
    /** Adapter capabilities (address context, supported chains). */
    readonly capabilities: TCapabilities;
    /** Factory that returns an Ethers `Provider` for a given chain. */
    getProvider: (params: {
        chain: EVMChainDefinition;
    }) => Promise<Provider> | Provider;
    /** Factory that returns an Ethers `Signer` for a given chain. */
    getSigner: (params: {
        chain: EVMChainDefinition;
    }) => Promise<Signer> | Signer;
    /** Optional runtime configuration for metrics and logging. */
    runtime?: Runtime | RuntimeOptions;
    /** Optional token registry for token resolution. */
    tokens?: TokenRegistry;
    /** Optional operational config (timeouts, retries, fees). */
    config?: OperationalConfig;
    /**
     * Whether to cache signer instances by chain ID.
     *
     * @defaultValue `true` when `addressContext` is `'developer-controlled'`
     */
    cacheSigners?: boolean;
    /**
     * Chain-switch behaviour applied before write operations.
     *
     * @remarks
     * When provided, the context exposes an `ensureChain` hook that
     * `createPrimitive` invokes before running a primitive that opted in
     * via `ensureChainBefore: true`. The hook reads the signer's current
     * chain and reconciles it with the target chain according to the chosen
     * `onMismatch` policy.
     *
     * Defaults applied by the factory helpers:
     * - `createEthersAdapterFromProvider` — `'prompt'`
     *   (EIP-1193 wallets can respond to `wallet_switchEthereumChain`).
     * - `createEthersAdapterFromPrivateKey` — `'skip'`
     *   (no wallet UI, chain pinned by the `JsonRpcProvider`).
     */
    chainSwitch?: ChainSwitchConfig;
}
/**
 * Runtime context for the Ethers adapter.
 *
 * @remarks
 * Extends the base adapter context with Ethers-specific accessors
 * for `Provider` and `Signer`. Instances are lazily created and
 * cached according to `AdapterContextOptions.cacheSigners`.
 *
 * @typeParam TCapabilities - Adapter capability constraints.
 *
 * @example
 ...
 * })
 *
 * const provider = await ctx.getProvider(Ethereum)
 * const signer = await ctx.getSigner(Ethereum)
 * ```
 */
interface AdapterContext<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> extends AdapterContext$1<TCapabilities> {
    /** Resolve a cached Ethers `Provider` for the given chain. */
    getProvider: (chain: EVMChainDefinition) => Promise<Provider>;
    /** Resolve a (possibly cached) Ethers `Signer` for the given chain. */
    getSigner: (chain: EVMChainDefinition) => Promise<Signer>;
    /**
     * Per-adapter nonce manager.
     *
     * @remarks
     * Each adapter context owns its own {@link EthersNonceManager}
     * instance, scoped to the lifetime of this context. This avoids the
     * cross-adapter contention and stale-state problems that the
     * previous module-level singleton (`ethersNonceManager`) caused
     * when multiple adapters in the same process shared a wallet
     * address. The `execute` primitive uses this manager exclusively;
     * external callers should use `resetClients()` (or rebuild the
     * context) rather than reaching into the manager directly.
     */
    readonly nonceManager: EthersNonceManager;
 ...
 * ```
 */
type EVMAdapter<TContext extends AdapterContext$1 = AdapterContext$1> = Adapter$1<TContext, EVMReadInput, EVMWriteInput, EVMChainDefinition>;
 ...
 *
 * @remarks
 * Used by the SCA pre-check (`assertSignerIsEoa`) to detect on-chain bytecode
 * at the prospective signer address before producing a Gateway burn-intent
 * signature. Gateway verifies signatures with plain `ecrecover` and does not
 * fall back to ERC-1271, so a smart-contract-account signer would silently
 * produce an unverifiable signature; this hook lets the binding catch that
 * case at signing time.
 *
 * Implementations should return `'0x'` (or `undefined`) for an EOA and the
 * raw deployed bytecode for an SCA. Errors are caught at the call site so
 * an RPC failure does not block the legacy signing path; see DEVX-2774.
 */
 ...
 * ```
 */
interface EVMActionRegistry extends BaseActionRegistry {
    'cctp.v2.depositForBurn': WriteAction<CCTPDepositForBurnInput>;
    'cctp.v2.receiveMessage': WriteAction<CCTPReceiveMessageInput>;
    'cctp.v2.customBurn': WriteAction<CCTPCustomBurnInput>;
    'cctp.v2.depositForBurnWithHook': WriteAction<CCTPDepositForBurnWithHookInput>;
    'cctp.v2.customBurnWithHook': WriteAction<CCTPCustomBurnWithHookInput>;
    'gateway.v1.deposit': WriteAction<GatewayDepositInput>;
    'gateway.v1.depositFor': WriteAction<GatewayDepositForInput>;
    'gateway.v1.depositWithPermit': WriteAction<GatewayDepositWithPermitInput>;
    'gateway.v1.depositWithAuthorization': WriteAction<GatewayDepositWithAuthorizationInput>;
    'gateway.v1.addDelegate': WriteAction<GatewayAddDelegateInput>;
    'gateway.v1.removeDelegate': WriteAction<GatewayRemoveDelegateInput>;
    'gateway.v1.isDelegate': ReadAction<GatewayIsDelegateInput, GatewayIsDelegateOutput>;
    'gateway.v1.initiateWithdrawal': WriteAction<GatewayInitiateWithdrawalInput>;
    'gateway.v1.withdraw': WriteAction<GatewayWithdrawInput>;
    'gateway.v1.withdrawingBalance': ReadAction<GatewayWithdrawingBalanceInput, GatewayWithdrawingBalanceOutput>;
    'gateway.v1.withdrawalBlock': ReadAction<GatewayWithdrawalBlockInput, GatewayWithdrawalBlockOutput>;
    'gateway.v1.gatewayBurn': WriteAction<GatewayBurnInput>;
    'gateway.v1.gatewayMint': WriteAction<GatewayMintInput>;
    'gateway.v1.signBurnIntents': ReadAction<GatewaySignBurnIntentsInput, GatewaySignatureOutput>;
    'swap.execute': WriteAction<ExecuteSwapInput>;
    'usdt.transfer': WriteAction<{
        to: string;
        amount: bigint;
    }>;
    'earn.deposit': WriteAction<EarnExecuteInput>;
    'earn.withdraw': WriteAction<EarnExecuteInput>;
    'earn.claimRewards': WriteAction<EarnExecuteInput>;
}
/**
...
interface CreateEVMActionsOptions {
     ...
     *
     * @remarks
     * Injected here so the `gateway.v1.signBurnIntents` binding can run an
     * EOA pre-check before producing a Gateway burn-intent signature. Gateway
     * verifies signatures with `ecrecover` and does not fall back to ERC-1271,
     * so an SCA signer would silently produce an unverifiable signature.
     *
     * When provided alongside `signTypedData`, the binding fetches bytecode
     * for the resolved signer address and throws `KitError(InputError.UNSUPPORTED_ACTION)`
     * with the delegate-pattern recipe when bytecode is present (and is not an
     * EIP-7702 delegation prefix). When omitted, the pre-check is skipped —
     * mirrors the legacy fix landed in #954 (DEVX-2774).
     *
     * @example
     * ```typescript
 ...
 * ```
 */
interface EVMEstimateRaw extends EVMFeeRaw$1 {
 ...
 * ```
 */
type EstimatePrimitive = PrimitiveFunction<EstimateInput, EstimateResult>;
...
type SimulatePrimitive = PrimitiveFunction<SimulateInput, SimulateResult>;
 ...
 * - `PrepareNativeInput` — plain native-token transfer; the adapter emits
 *   `sendTransaction` with no calldata.
 *
 * The discriminant is `type`: `'contract'` (or omitted) vs `'raw'` vs `'native'`.
 */
type PrepareInput = PrepareContractInput | PrepareRawInput | PrepareNativeInput;
/** A prepared EVM transaction with estimate, simulate, and execute methods. */
interface PreparedEVMTransaction extends PreparedTransactionBase {
    ...
    execute(overrides?: GasOverrides): Promise<ExecuteResult>;
}
/** Prepare result containing one or more prepared EVM transactions. */
type EVMPrepareResult = PrepareResult<PrepareInput, RawTransaction, unknown, PreparedEVMTransaction>;
 ...
 */
type LegacyCompatibleAdapter<TAdapter extends StandardAdapter = StandardAdapter> = Omit<TAdapter, keyof Adapter | 'prepare'> & Omit<LegacyCompatibleAdapterBase, 'prepare' | 'capabilities'> & {
    /**
...
/**
 * Input parameters for reading contract data.
 *
 * @example
 * ```typescript
 * const input: ReadContractInput = {
 *   address: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',
 *   abi: ERC20_ABI,
 *   functionName: 'balanceOf',
 *   args: ['0x...'],
...
interface ReadContractInput {
    readonly address: `0x${string}`;
    /** ABI items — kept as `readonly unknown[]` to match `EVMReadInput` and avoid the ethers `InterfaceAbi` union leaking into shared types. Cast to `InterfaceAbi` happens inside the primitive. */
    readonly abi: readonly unknown[];
    readonly functionName: string;
    readonly args?: readonly unknown[] | undefined;
}
/** Primitive function type for reading contract data. */
type ReadPrimitive = GenericPrimitiveFunction<ReadContractInput>;
/**
...
/** Raw EVM transaction receipt from ethers. */
interface EVMReceiptRaw {
    /** The ethers transaction receipt object. */
    readonly receipt: TransactionReceipt;
}
/** Result from waiting for transaction confirmation. */
type WaitForTransactionResult = WaitResult<EVMReceiptRaw>;
/** Primitive function for waiting for transaction confirmation. */
type WaitForTransactionPrimitive = PrimitiveFunction<WaitForTransactionInput, WaitForTransactionResult>;
/**
...
/**
 * Raw EVM fee data containing gas estimate and pricing information.
 *
 * @remarks
 * Contains either EIP-1559 fields (maxFeePerGas, maxPriorityFeePerGas) or
 * legacy field (gasPrice) depending on chain support.
 *
 * @example
 * ```typescript
 * const feeRaw: EVMFeeRaw = {
...
/**
 * Input parameters for calculating transaction fees.
 *
 * @example
 * ```typescript
 * const input: CalculateFeeInput = {
 *   gasUnits: 21000n,
 *   bufferBasisPoints: 2000n,
 * }
 * ```
 */
...
/** Fee estimate result containing EVM-specific raw fee data. */
type EVMFeeEstimate = FeeEstimate<EVMFeeRaw>;
/**
 * Primitive function type for calculating transaction fees.
 *
 * @remarks
 * Accepts gas units and optional buffer, returns a complete fee estimate
 * with both EIP-1559 and legacy pricing fields.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
type CalculateFeePrimitive = (input: CalculateFeeInput, operation: OperationMeta, invocation?: InvocationMeta) => Promise<EVMFeeEstimate>;
/**
...
type ExecutePrimitive = PrimitiveFunction<ExecuteTransactionInput, EVMExecuteResult>;
/**
...
/**
 * Input for signing EIP-712 typed data.
 *
 * @typeParam Types - The typed data structure definition.
...
interface SignTypedDataInput<Types extends Record<string, TypedDataField[]> = Record<string, TypedDataField[]>, Message extends Record<string, unknown> = Record<string, unknown>> {
    ...
    /** The typed data structure definition. */
    readonly types: Types;
    /** The primary type name from the types object. */
    readonly primaryType: keyof Types & string;
    /** The message payload to sign. */
    readonly message: Message;
}
/** The signature result from signing typed data (hex string). */
type SignTypedDataResult = `0x${string}`;
/** Primitive function for signing EIP-712 typed data. */
type SignTypedDataPrimitive = PrimitiveFunction<SignTypedDataInput, SignTypedDataResult>;
/**
...
/**
 * Primitive function type for preparing EVM transactions.
 *
 * @remarks
 ...
 * ```
 */
type PreparePrimitive = (input: PrepareInput | PrepareInput[], operation: OperationMeta, invocation?: InvocationMeta) => Promise<EVMPrepareResult>;
/**
...
/**
 * The clean ethers adapter interface (new API).
 *
 * @remarks
 * Derived from {@link AssembledAdapterShape} — the standard adapter shape
 * returned by `assembleAdapter` — plus ethers-specific extras.
 *
 ...
 */
type EthersAdapterCore<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = AssembledAdapterShape<AdapterContext<TCapabilities>, 'evm', GenericPrimitiveFunction<ReadContractInput>, PreparePrimitive, WaitForTransactionPrimitive, EVMActionRegistry> & {
...
/**
 * The public ethers adapter type — clean interface + legacy compat methods.
 *
 * @remarks
 ...
 *   `typeof adapter`.
 */
type EthersAdapter<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1> = LegacyCompatibleAdapter<EthersAdapterCore<TCapabilities>>;
/**
 ...
 * ```
 */
declare function createAdapter<const TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1>(options: AdapterOptions<TCapabilities>): EthersAdapter<TCapabilities>;
/**
 ...
 * ```
 */
declare function createAdapterFromContext<TCapabilities extends AdapterCapabilities$1 = AdapterCapabilities$1>(ctx: AdapterContext<TCapabilities>): EthersAdapter<TCapabilities>;
...
interface ActionFactoryDeps {
    readonly ctx: AdapterContext;
    readonly read: GenericPrimitiveFunction<ReadContractInput>;
...
/**
 * Parameters accepted by all ethers adapter factories.
 *
 * @remarks
 * `getProvider` receives both `chainId` (for backwards compatibility) and the
 * full `chain` definition so implementors can access RPC URLs, explorer links,
 * or any other chain metadata when constructing their provider.
 *
 * @example
 * ```typescript
 * import type { BaseAdapterParams } from '@circle-fin/adapter-ethers-v6/next'
 * import { JsonRpcProvider } from 'ethers'
 *
 ...
 * ```
 */
interface BaseAdapterParams {
    capabilities?: Partial<AdapterCapabilities$1> | undefined;
    getProvider?: (params: {
        chainId: number;
        chain: EVMChainDefinition;
    }) => Promise<Provider> | Provider;
    ...
    config?: OperationalConfig;
    /**
     * Chain-switch policy for write operations. Each factory applies its
     * own default when omitted (`prompt` for provider/EIP-1193 flows,
     * `skip` for private-key/server flows).
     */
    chainSwitch?: ChainSwitchConfig;
}

/**
 * Parameters for creating an Ethers adapter from a private key.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
interface CreateEthersAdapterFromPrivateKeyParams extends BaseAdapterParams {
    privateKey: string;
}
/**
 ...
 * ```
 */
declare function createEthersAdapterFromPrivateKey(params: CreateEthersAdapterFromPrivateKeyParams): EthersAdapter;

/**
 * Parameters for creating an Ethers adapter from a provider.
 *
 * @remarks
 * The `provider` field accepts either:
 * - An **EIP-1193 provider** (e.g. `window.ethereum`) — wrapped via
 *   ethers' `BrowserProvider` per chain. Suited for browser wallets.
 * - An ethers **`Provider`** (e.g. `JsonRpcProvider`) — used directly
 *   without wrapping. Suited for server-side use cases where the
 *   provider is already a fully-fledged ethers Provider with its own
 *   account list. See PR #853 round-2 review (finding N8).
 *
 * @example
 * ```typescript
 ...
 * import { JsonRpcProvider } from 'ethers'
 *
 * // Server-side / ethers Provider
 * const provider = new JsonRpcProvider(process.env.RPC_URL)
 * const adapter = createEthersAdapterFromProvider({ provider })
 * ```
 */
interface CreateEthersAdapterFromProviderParams extends BaseAdapterParams {
 ...
 * ```
 */
declare function createEthersAdapterFromProvider(params: CreateEthersAdapterFromProviderParams): EthersAdapter;
...
export type { ActionDispatch, AdapterCapabilities$1 as AdapterCapabilities, AddressField$1 as AddressField, BaseAdapterParams, CalculateFeeInput, CalculateFeePrimitive, CapabilityAwareOperationMeta, ChainDefinition, CreateEthersAdapterFromPrivateKeyParams, CreateEthersAdapterFromProviderParams, EVMActionRegistry, EVMChainDefinition, EVMExecuteResult, EstimatePrimitive, EstimateResult, EthersAdapter, AdapterContext as EthersAdapterContext, AdapterContextOptions as EthersAdapterContextOptions, EthersAdapterCore, ExecutePrimitive, ExtractAddressContext$1 as ExtractAddressContext, GasOverrides, OperationalConfig, PrepareContractInput, PreparePrimitive, PreparedEVMTransaction, RawTransaction, ReadContractInput, ReadPrimitive, RetryOptions, Runtime, RuntimeOptions, SignTypedDataInput, SignTypedDataPrimitive, SignTypedDataResult, SimulatePrimitive, SimulateResult, TokenDefinition, TokenRegistry, TokenRegistryOptions, TransactionConfig, WaitForTransactionInput, WaitForTransactionPrimitive, WaitForTransactionResult };
