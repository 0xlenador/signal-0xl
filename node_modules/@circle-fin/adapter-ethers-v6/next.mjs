...
import { Interface, JsonRpcProvider, Wallet, BrowserProvider } from 'ethers';
 ...
 */ const prepareInputSchema = z.preprocess((input)=>{
...
/**
 * Build transaction parameters from raw transaction and gas overrides.
 *
 * @remarks
 * Shared utility for EVM adapters that constructs the base transaction
 * object and applies optional gas/nonce overrides.
 *
 * @param raw - The raw transaction containing base fields (to, from, data, value).
 * @param overrides - Optional gas and nonce overrides to apply.
 * @returns A transaction parameters object with base fields and any applied overrides.
 *
 * @example
 * ```typescript
 * import { buildEVMTxParams } from '@core/adapter-evm-base'
 *
 * const txParams = buildEVMTxParams(
 *   {
 ...
 *     value: 0n,
 *   },
 *   { gas: 21_000n, maxFeePerGas: 30_000_000_000n },
 * )
 * ```
 */ function buildEVMTxParams(raw, overrides) {
    const txParams = {
        to: raw.to,
        from: raw.from,
        data: raw.data,
    ...
    if (overrides?.gas !== undefined) txParams.gas = overrides.gas;
    if (overrides?.maxFeePerGas !== undefined) txParams.maxFeePerGas = overrides.maxFeePerGas;
    if (overrides?.maxPriorityFeePerGas !== undefined) txParams.maxPriorityFeePerGas = overrides.maxPriorityFeePerGas;
    if (overrides?.gasPrice !== undefined) txParams.gasPrice = overrides.gasPrice;
    if (overrides?.nonce !== undefined) txParams.nonce = overrides.nonce;
    return txParams;
}
...
function getSendCapableProvider(signer) {
...
async function readChainId(provider) {
    try {
        if (typeof provider.getNetwork === 'function') {
            ...
            const value = typeof network.chainId === 'bigint' ? Number(network.chainId) : network.chainId;
            if (Number.isFinite(value)) {
                return value;
            }
        }
    } catch  {
    // Fall through to the RPC probe.
    }
    try {
    ...
    // The wallet may simply be offline. Propagate as "unknown" so the
    // caller falls through to the mismatch policy.
    }
    return undefined;
}
/**
 * Create an `ensureChain` function for the ethers `/next` adapter.
 *
 * @remarks
 * The returned function is idempotent: if the signer's provider already
 * reports the target chain ID, no prompt is surfaced. Behaviour on
 * mismatch depends on the `onMismatch` option:
 *
 * - `'prompt'` — issue `wallet_switchEthereumChain`, falling back to
 *   `wallet_addEthereumChain` on EIP-3085 code `4902` and retrying the
 *   switch once.
 * - `'throw'` — raise a `CHAIN_MISMATCH` `KitError` without prompting.
 * - `'skip'` — never called; factories should simply omit `ensureChain`.
 *
 * The factory validates the target chain type and translates EIP-1193
 * user rejection into a typed `CHAIN_SWITCH_REJECTED` error so callers
 * can distinguish cancellation from wallet faults.
 */ function createEnsureChain(options) {
    const { getSigner, onMismatch = 'prompt' } = options;
    return async function ensureChain(chain) {
        ...
        if (chain.type !== 'evm') {
            throw new KitError({
                ...InputError.INVALID_CHAIN,
                recoverability: 'FATAL',
                message: `EthersAdapter.ensureChain only supports EVM chains. ` + `Received: ${String(chain.type)} (${chain.name})`
            });
        }
        const evmChain = chain;
        const signer = await getSigner(evmChain);
        const rpcProvider = getSendCapableProvider(signer);
        // Server-side / private-key signers resolve their chain via the
        // provider passed at construction time, so there is nothing to
        // switch. The adapter factory skips this hook for those contexts,
        // but handle it defensively in case a caller wires us up manually.
        if (rpcProvider === null) {
            return;
        }
        const currentChainId = await readChainId(rpcProvider);
        ...
        await switchOrAddChain(rpcProvider, evmChain);
    };
}
async function switchOrAddChain(rpcProvider, evmChain) {
    const targetChainIdHex = `0x${evmChain.chainId.toString(16)}`;
    try {
        await rpcProvider.send('wallet_switchEthereumChain', [
            {
        ...
        if (isUserCancellationError(error)) {
            throw buildSwitchRejectedError(evmChain, error, false);
        }
        // EIP-3085: the wallet does not know about this chain yet. Offer to
        // add it and retry the switch once. Surfacing a single typed error
        // on failure avoids the caller seeing a duplicate add+switch prompt
        // with no useful metadata.
        if (isUnrecognizedChainError(error)) {
            await addAndRetrySwitch(rpcProvider, evmChain, targetChainIdHex);
            return;
        }
        throw buildUnrecognizedChainError(evmChain, error, false);
    }
}
async function addAndRetrySwitch(rpcProvider, evmChain, targetChainIdHex) {
    // EIP-3085 requires at least one RPC URL. If the chain definition is
    // missing endpoints we cannot build a valid `wallet_addEthereumChain`
    // request; surface a typed, descriptive error instead of letting the
    // wallet reject the call with an opaque "invalid params" message.
    if (evmChain.rpcEndpoints.length === 0) {
        throw new KitError({
            ...InputError.UNRECOGNIZED_CHAIN,
            recoverability: 'FATAL',
            message: `Cannot add ${evmChain.name} (${String(evmChain.chainId)}) to the ` + `wallet: the chain definition has no RPC endpoints. EIP-3085 ` + `requires at least one 'rpcUrls' entry.`,
            cause: {
                trace: {
                    targetChainId: evmChain.chainId,
                    targetChainName: evmChain.name
    ...
    try {
        await rpcProvider.send('wallet_addEthereumChain', [
            buildAddChainParams(evmChain)
        ]);
        await rpcProvider.send('wallet_switchEthereumChain', [
            {
                chainId: targetChainIdHex
        ...
        if (isUserCancellationError(retryError)) {
            throw buildSwitchRejectedError(evmChain, retryError, true);
        }
        throw buildUnrecognizedChainError(evmChain, retryError, true);
    }
}
function buildSwitchRejectedError(evmChain, rawError, duringAdd) {
    const action = duringAdd ? 'add or switch to' : 'switch to';
    return new KitError({
        ...InputError.CHAIN_SWITCH_REJECTED,
        recoverability: 'FATAL',
        message: `User rejected the request to ${action} ${evmChain.name} ` + `(${String(evmChain.chainId)}).`,
        cause: {
            trace: {
                targetChainId: evmChain.chainId,
                targetChainName: evmChain.name,
                rawError: redactRawError(rawError)
            }
        }
    });
}
function buildUnrecognizedChainError(evmChain, rawError, duringAdd) {
    const message = duringAdd ? `Failed to add or switch the wallet to ${evmChain.name} ` + `(${String(evmChain.chainId)}). The wallet may not support adding ` + `custom chains or the RPC configuration is invalid.` : `Failed to switch the wallet to ${evmChain.name} ` + `(${String(evmChain.chainId)}). The wallet may not recognize the ` + `chain or is unable to add it.`;
    return new KitError({
        ...InputError.UNRECOGNIZED_CHAIN,
        recoverability: 'FATAL',
        message,
...
function isUnrecognizedChainError(error) {
    ...
    if (withCode.code === UNRECOGNIZED_CHAIN_ID_ERROR_CODE) {
        return true;
    }
    // Ethers often wraps the EIP-1193 error; inspect the nested payload
    // to avoid false negatives on MetaMask/Wagmi bridges.
    return withCode.data?.originalError?.code === UNRECOGNIZED_CHAIN_ID_ERROR_CODE;
}
function buildAddChainParams(chain) {
    // Callers (addAndRetrySwitch) must guard against empty rpcEndpoints
    // before invoking this helper — see the EIP-3085 precondition check.
    const [firstEndpoint] = chain.rpcEndpoints;
    if (firstEndpoint === undefined) {
        throw new Error('buildAddChainParams invariant: rpcEndpoints must be non-empty');
    }
    return {
 ...
 * @throws {KitError} INPUT_VALIDATION_FAILED when called on developer-controlled adapters
 * @throws {KitError} INPUT_INVALID_CHAIN when chain type is not 'evm'
 * @throws {KitError} RPC_ENDPOINT_ERROR when wallet returns no address
 *
 * @remarks
 ...
 * @example
 * ```typescript
 * import { createGetAddress } from '@circle-fin/adapter-ethers-v6/next'
 ...
 */ function createGetAddress(options) {
    const { isDeveloperControlled, getSigner } = options;
    return async function getAddress(chain) {
        ...
        if (chain.type !== 'evm') {
            throw new KitError({
                ...InputError.INVALID_CHAIN,
                recoverability: 'FATAL',
                message: `EthersAdapter only supports EVM chains. Received: ${chain.type} (${chain.name})`
            });
        }
...
// Utility Functions
// ============================================================================
function isEthersLikeError(error) {
    return error !== null && typeof error === 'object' && 'code' in error && typeof error.code === 'string';
}
function extractEthersDetails(error) {
    // Pass the redacted error shape into `cause.trace` rather than the
    // ethers-injected object graph (provider URL, request payload, the
    // signing wallet, etc.). The structured `code` / `shortMessage` we
    // attach alongside is enough to drive downstream classification.
    //
    // ethers v6's `makeError()` formats info keys directly into both
    // `message` and `shortMessage` (e.g. `requestUrl="…/MY_KEY"`), so
    // run the field through the operator-configured message redactor.
    // Symmetry with the parallel `cause.trace.rawError.message` channel
    // (also redacted inside `redactRawError`) means a single
    // `setMessageRedactor([...])` call at app boot scrubs both copies.
    // Without the wrap, telemetry pipelines that JSON-serialise
    // `cause.trace` would still leak the secret on `shortMessage` even
    // after `rawError` was redacted.
    return {
        rawError: redactRawError(error),
        code: error.code,
        shortMessage: redactErrorMessage(error.shortMessage ?? error.message ?? '')
    };
}
function getReason(error) {
    // ethers v6 formats info keys (e.g. `requestUrl="…/MY_KEY"`) into
    // `reason` / `shortMessage` / `message`. This value flows into the
    // user-facing `KitError.message` via the handler dispatch
    // (`createSimulationFailedError(chain, reason)`, etc.), so redact it
    // through the operator-configured message redactor before it escapes.
    return redactErrorMessage(error.reason ?? error.shortMessage ?? error.message ?? 'Unknown');
}
function extractAddressFromUnconfiguredName(error) {
    const info = error.info;
    if (typeof info === 'object' && info !== null && typeof info['value'] === 'string') {
        return info['value'];
    }
    const msg = error.message ?? error.shortMessage ?? error.reason ?? '';
    const valueMatch = /value=["']([^"']*)["']/.exec(msg);
    return valueMatch?.[1] ?? (msg || 'unknown');
}
function createErrorHandlers() {
    return {
        ACTION_REJECTED: (_e, chain, details)=>{
            throw createUserRejectedError(chain, details);
        },
        CANCELLED: (_e, chain, details)=>{
            throw createUserRejectedError(chain, details);
        },
        INVALID_ARGUMENT: (error, _chain, _details, reason)=>{
            // `reason` is already redacted (see getReason); redact the parallel
            // shortMessage/message path too so the message arg can't leak a key.
            const shortMsg = redactErrorMessage(error.shortMessage ?? error.message ?? reason);
            throw createValidationFailedError('argument', reason, shortMsg);
        },
        MISSING_ARGUMENT: (_e, _chain, _details, reason)=>{
            throw createValidationFailedError('argument', 'missing', reason);
        },
        UNEXPECTED_ARGUMENT: (_e, _chain, _details, reason)=>{
            throw createValidationFailedError('argument', 'unexpected', reason);
        },
        VALUE_MISMATCH: (_e, _chain, _details, reason)=>{
            throw createValidationFailedError('argument', 'value', reason);
        },
        NUMERIC_FAULT: (_e, _chain, _details, reason)=>{
            throw createValidationFailedError('argument', 'numeric', reason);
        },
        UNCONFIGURED_NAME: (error, chain)=>{
            const address = extractAddressFromUnconfiguredName(error);
            throw createInvalidAddressError(address, chain, 'resolved ENS or configured name');
        },
        INSUFFICIENT_FUNDS: (_e, chain, details)=>{
            throw createInsufficientGasError(chain, undefined, details);
        },
        CALL_EXCEPTION: (_e, chain, details, reason)=>{
            throw createSimulationFailedError(chain, reason, details);
        },
        UNPREDICTABLE_GAS_LIMIT: (_e, chain, details, reason)=>{
            throw createSimulationFailedError(chain, reason, details);
        },
        NONCE_EXPIRED: (_e, chain, details, reason)=>{
            throw createRpcNonceError(chain, reason, details);
        },
        REPLACEMENT_UNDERPRICED: (_e, chain, details, reason)=>{
            throw createRpcNonceError(chain, reason, details);
        },
        TRANSACTION_REPLACED: (_e, chain, details, reason)=>{
            throw createRpcNonceError(chain, reason, details);
        },
        NETWORK_ERROR: (error, chain, details)=>{
            // ethers v6 emits NETWORK_ERROR for two structurally different
            // conditions: a transient transport failure (RETRYABLE) and a
            // deterministic chainId mismatch (FATAL — wallet is on chain X,
            // adapter wants chain Y; no amount of retrying will help). The
            // chainId-mismatch payload always carries the literal phrase
            // `network changed: <expected> => <actual>` on `shortMessage`.
            // Detect it explicitly so the runtime doesn't burn 7+ seconds
            // on retries before surfacing the (deterministic) failure.
            // (PR #853 round-2 review, finding N2.)
            const msg = error.shortMessage ?? error.message ?? '';
            // Detect the literal phrase first with a trivial pattern, then
            // pull the actual chain id from the `=> <id>` suffix in a second
            // pass. Splitting the two avoids the alternation-heavy single
            // pattern Sonar flags as overly complex (`\d+|\w+` × 2 inside a
            // nested optional group).
            if (/network changed/i.test(msg)) {
                const arrowMatch = /=>\s*(\w+)/.exec(msg);
                const actual = arrowMatch?.[1];
                throw createChainMismatchError(chain, actual, {
        ...
        TIMEOUT: (_e, chain, details, reason)=>{
            throw createNetworkTimeoutError(chain, reason, details);
        },
        OFFCHAIN_FAULT: (_e, chain, details)=>{
            throw createNetworkConnectionError(chain, details);
        },
        SERVER_ERROR: (_e, chain, details)=>{
            throw createRpcEndpointError(chain, details);
        },
        BAD_DATA: (_e, chain, details)=>{
            throw createRpcEndpointError(chain, details);
        },
        NOT_IMPLEMENTED: (_e, chain, details)=>{
            throw createRpcEndpointError(chain, details);
        },
        UNSUPPORTED_OPERATION: (_e, chain, details)=>{
            throw createRpcEndpointError(chain, details);
        },
        BUFFER_OVERRUN: (_e, chain, details)=>{
            throw createRpcEndpointError(chain, details);
        }
    };
}
const ERROR_HANDLERS = createErrorHandlers();
function handleEthersErrorByCode(error, chain) {
    const code = error.code;
    const details = extractEthersDetails(error);
    const reason = getReason(error);
    const handler = code === undefined ? undefined : ERROR_HANDLERS[code];
    if (handler === undefined) {
        return;
    }
    handler(error, chain, details, reason);
// UNKNOWN_ERROR or unhandled code falls through to generic fallback
}
// ============================================================================
// Main Error Normalizer
// ============================================================================
/**
 * Error normalizer for Ethers adapter errors.
 *
 * @remarks
 * Composes ethers-specific error code handling with shared EVM message
 * pattern matching from `@core/adapter-evm-base`.
 *
 * | Phase | Ethers Code | KitError |
 * |-------|-------------|----------|
 * | 1 | `ACTION_REJECTED` | `INPUT_USER_CANCELLED` |
 * | 1 | `CANCELLED` | `INPUT_USER_CANCELLED` |
 * | 2 | `INVALID_ARGUMENT` | `INPUT_VALIDATION_FAILED` |
 * | 2 | `MISSING_ARGUMENT` | `INPUT_VALIDATION_FAILED` |
 * | 2 | `UNEXPECTED_ARGUMENT` | `INPUT_VALIDATION_FAILED` |
 * | 2 | `VALUE_MISMATCH` | `INPUT_VALIDATION_FAILED` |
 * | 2 | `NUMERIC_FAULT` | `INPUT_VALIDATION_FAILED` |
 * | 2 | `UNCONFIGURED_NAME` | `INPUT_INVALID_ADDRESS` |
 * | 3 | `INSUFFICIENT_FUNDS` | `BALANCE_INSUFFICIENT_GAS` |
 * | 4 | `CALL_EXCEPTION` | `ONCHAIN_SIMULATION_FAILED` |
 * | 4 | `UNPREDICTABLE_GAS_LIMIT` | `ONCHAIN_SIMULATION_FAILED` |
 * | 5 | `NONCE_EXPIRED` | `RPC_NONCE_ERROR` |
 * | 5 | `REPLACEMENT_UNDERPRICED` | `RPC_NONCE_ERROR` |
 * | 5 | `TRANSACTION_REPLACED` | `RPC_NONCE_ERROR` |
 * | 6 | `NETWORK_ERROR` | `NETWORK_CONNECTION_FAILED` |
 * | 6 | `TIMEOUT` | `NETWORK_TIMEOUT` |
 * | 6 | `OFFCHAIN_FAULT` | `NETWORK_CONNECTION_FAILED` |
 * | 7 | `SERVER_ERROR` | `RPC_ENDPOINT_ERROR` |
 * | 7 | `BAD_DATA` | `RPC_ENDPOINT_ERROR` |
 * | 7 | `NOT_IMPLEMENTED` | `RPC_ENDPOINT_ERROR` |
 * | 7 | `UNSUPPORTED_OPERATION` | `RPC_ENDPOINT_ERROR` |
 * | 7 | `BUFFER_OVERRUN` | `RPC_ENDPOINT_ERROR` |
 * | — | `UNKNOWN_ERROR` | Falls through to pattern matching / generic |
 * | — | Message patterns | Shared EVM pattern matching |
 * | — | Other errors | `RPC_ENDPOINT_ERROR` |
 */ const errorNormalizer = createEVMErrorNormalizer({
    handleFrameworkError: (error, chain)=>{
        if (isEthersLikeError(error)) {
            handleEthersErrorByCode(error, chain);
            // Both `msg` and `reason` can flow into a user-facing KitError.message
            // inside matchEVMMessagePatterns, and ethers embeds the request URL
            // (and any embedded key) in them — redact before matching.
            const msg = redactErrorMessage(error.message ?? error.shortMessage ?? '');
            const details = extractEthersDetails(error);
            const reason = redactErrorMessage(error.reason ?? error.shortMessage ?? error.message ?? '');
            matchEVMMessagePatterns(msg, chain, details, reason);
        }
    }
});
 ...
 */ const MEMORY_CONFIG = {
    /** Maximum number of states to keep in memory */ MAX_STATES: 1000,
    /** Time-to-live for unused states in milliseconds (30 minutes) */ STATE_TTL_MS: 30 * 60 * 1000,
    /** Interval for cleanup checks in milliseconds (5 minutes) */ CLEANUP_INTERVAL_MS: 5 * 60 * 1000
};
/**
 ...
 *
 * @internal
 */ const SHARED_NONCE_STATES = new Map();
/**
 ...
 */ function performSharedCleanup() {
    const now = Date.now();
    const keysToDelete = [];
    for (const [key, state] of SHARED_NONCE_STATES.entries()){
    ...
    if (SHARED_NONCE_STATES.size > MEMORY_CONFIG.MAX_STATES) {
        ...
        for(let i = 0; i < excessCount; i++){
            ...
            if (entry) {
                SHARED_NONCE_STATES.delete(entry[0]);
            }
        }
    }
    if (SHARED_NONCE_STATES.size === 0) {
        stopSharedCleanupTimer();
    }
}
/**
 ...
 */ function ensureSharedCleanupTimer() {
    ...
    if (typeof SHARED_CLEANUP_TIMER === 'object' && 'unref' in SHARED_CLEANUP_TIMER) {
        SHARED_CLEANUP_TIMER.unref();
    }
}
/**
 ...
 */ class EthersNonceManager {
   ...
   */ clear(chainId, address) {
        ...
        // Only clear keys this instance owns. Another adapter targeting the
        // same key keeps its progress.
   ...
   * ```
   */ clearAll() {
        // Only clear keys this instance has touched — other adapters keep
        // their progress on keys they own.
   ...
   */ shouldRetryNonceError(error, userProvidedNonce) {
        ...
        if (error instanceof Error) {
            ...
            if (hasNoncePattern) return true;
            // Check for provider-specific error codes
            const errorWithCode = error;
            if (errorWithCode.code && NONCE_ERROR_CODES.includes(errorWithCode.code)) {
...
const ADAPTER_NAME = 'ethers';
const ADAPTER_VERSION = '6.0.0';
/**
 * Create an Ethers adapter context with provider/signer caching.
 *
 * @remarks
 * Provider instances are cached by `chain.chainId`. Signer instances
 * are cached when the adapter is developer-controlled (or when
 * `cacheSigners` is explicitly set), ensuring a single signer per
 * chain for the lifetime of the context.
 *
 * Call {@link AdapterContext.resetClients | resetClients()} to
 * invalidate all cached providers and signers.
 *
 * @typeParam TCapabilities - Adapter capability constraints.
 * @param options - Context configuration including provider/signer factories.
 * @returns A fully initialized {@link AdapterContext}.
 *
 * @example
 * ```typescript
 * import { createAdapterContext } from '@circle-fin/adapter-ethers-v6/next'
 *
 * const ctx = createAdapterContext({
 *   capabilities: { addressContext: 'user-controlled', supportedChains: [] },
 *   getProvider: ({ chain }) => new JsonRpcProvider(rpcUrl, chain.chainId),
 *   getSigner: ({ chain }) => new Wallet(privateKey, provider),
 * })
 * ```
 */ function createAdapterContext(options) {
    const { capabilities } = options;
    const isDeveloperControlled = capabilities.addressContext === 'developer-controlled';
    const shouldCacheSigners = options.cacheSigners ?? isDeveloperControlled;
    const providerCache = createAsyncCache({
        getCacheKey: ({ chain })=>chain.chainId,
        create: async ({ chain })=>{
            const result = await Promise.resolve(options.getProvider({
                chain
            }));
            return result;
        }
    });
    const signerCache = createAsyncCache({
        getCacheKey: ({ chain })=>shouldCacheSigners ? chain.chainId : undefined,
        create: async ({ chain })=>{
            const result = await Promise.resolve(options.getSigner({
                chain
            }));
            return result;
        }
    ...
    // Each adapter context owns its own `EthersNonceManager` handle.
    // The handle is per-adapter for lifecycle isolation (`clear` /
    // `clearAll` scope to the keys this adapter has touched), but the
    // underlying nonce state is held in a process-wide registry keyed by
    // `(chainId, address)` so two adapters that share a wallet
    // coordinate their counters and avoid on-chain "nonce too low"
    // collisions. See `EthersNonceManager` and PR #853 N0.
    const nonceManager = new EthersNonceManager();
    function resetClients() {
        providerCache.clear();
        signerCache.clear();
        // Clearing the per-adapter nonce manager is part of "reset" — the
        // next allocation will re-initialise from the provider's pending
        // count, matching the freshly-rebuilt provider/signer state.
        nonceManager.clearAll();
    }
    // ─────────────────────────────────────────────────────────────────────────
    // Chain-switch Hook
    //
    // Expose `ensureChain` when the caller configured a non-skip policy so
    // `createPrimitive` (with `ensureChainBefore: true`) can reconcile the
    // signer's active chain before prompting. When `onMismatch === 'skip'`
    // we omit the hook entirely so middleware fully short-circuits.
    // ─────────────────────────────────────────────────────────────────────────
    const chainSwitchMode = options.chainSwitch?.onMismatch ?? 'prompt';
...
// ============================================================================
/**
 * Create a read primitive for calling view/pure contract functions.
 *
 * @param ctx - The Ethers adapter context.
 * @returns A read primitive function.
 *
 * @remarks
 * Encodes calldata via `Interface`, executes via `provider.call`, and decodes
 * the return value — fully typed without dynamic property access.
 *
 * @example
 * ```typescript
 * import { createRead } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const read = createRead(ctx)
 * const decimals = await read(
 ...
 */ function createRead(ctx) {
    return createPrimitive({
        ...
        execute: async (input, executeCtx)=>{
            const provider = await ctx.getProvider(executeCtx.chain);
            // Cast is safe: callers always pass ABI item arrays; InterfaceAbi includes
            // a string overload that never appears at runtime in this SDK.
            const iface = new Interface(input.abi);
            const calldata = iface.encodeFunctionData(input.functionName, input.args ?? []);
            const rawResult = await provider.call({
                to: input.address,
                data: calldata
            });
            const decoded = iface.decodeFunctionResult(input.functionName, rawResult);
...
/**
 * Create a waitForTransaction primitive for confirming transactions.
 *
 * @param ctx - The adapter context.
 * @returns A primitive function for waiting for transaction confirmation.
 *
 * @example
 * ```typescript
 * import { createWaitForTransaction } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 ...
 */ function createWaitForTransaction(ctx) {
    return createPrimitive({
        ...
        adapterContext: ctx,
        requireAddress: false,
        // Opt out of retry middleware. ethers' inner `waitForTransaction`
        // already polls/retries up to the timeout; a middleware retry on a
        // transient failure doubles the effective deadline and can mask the
        // typed NETWORK_TIMEOUT. Matches the viem and Solana siblings.
        retryable: false,
        ...
        execute: async (input, executeCtx)=>{
            ...
            // aborted, then race the (signal-less) ethers wait against the signal.
            // Abort stops the local wait only; the tx keeps settling on-chain.
            throwIfAborted(signal, 'waitForTransaction');
            const provider = await ctx.getProvider(chain);
            ...
            const receipt = await raceAbort(provider.waitForTransaction(input.txId, confirmations, timeout), signal, 'waitForTransaction');
            if (receipt === null) {
                throw createValidationFailedError('txId', input.txId, 'Transaction receipt not found. Transaction may have been dropped or not yet mined.');
            }
            const effectiveGasPrice = 'effectiveGasPrice' in receipt && typeof receipt.effectiveGasPrice === 'bigint' ? receipt.effectiveGasPrice : receipt.gasPrice;
...
/**
 * Create a calculateFee primitive for estimating transaction costs.
 *
 * @param ctx - The Ethers adapter context.
 * @returns A calculateFee primitive function.
 *
 * @remarks
 * Supports both EIP-1559 (maxFeePerGas) and legacy (gasPrice) fee structures.
 * Applies a configurable buffer to account for fee volatility.
 *
 * @example
 * ```typescript
 * import { createCalculateFee } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const calculateFee = createCalculateFee(ctx)
 *
 * const result = await calculateFee(
 *   { gasUnits: 21000n, bufferBasisPoints: 2000n },
 *   { chain: Ethereum },
 * )
 * ```
 */ function createCalculateFee(ctx) {
    async function fetchFeeData(chain) {
        ...
        if (feeData.maxFeePerGas != null) {
            // EIP-1559 path. Some providers intermittently return `null` for
            // `maxPriorityFeePerGas`; previously we silently fell back to `0n`
            // (then floored to `priorityFloor`), which produced fees too low for
            // mainnet-scale chains and masked a misconfigured RPC. Surface the
            // RPC gap explicitly so callers can retry / switch endpoints.
            if (feeData.maxPriorityFeePerGas == null) {
                throw new KitError({
                    ...RpcError.ENDPOINT_ERROR,
                    recoverability: 'RETRYABLE',
                    message: `RPC returned null for maxPriorityFeePerGas on chain ` + `${chain.name}. The endpoint may be degraded; retry or switch ` + `providers.`
                });
            }
            ...
            // `maxFeePerGas` to match produces an inverted pair that ethers'
            // provider validation and the chain itself both reject. Clamp
        ...
        // Legacy path. If `gasPrice` is also null we have zero signal from the
        // RPC, so bailing out is the only safe option: `0n` would submit a
        // transaction at ~1.5 gwei (the priority floor) on legacy chains, far
        // below mainnet-typical rates and effectively unsendable.
        if (feeData.gasPrice == null) {
            throw new KitError({
                ...RpcError.ENDPOINT_ERROR,
                recoverability: 'RETRYABLE',
                message: `RPC returned null for both maxFeePerGas and gasPrice on chain ` + `${chain.name}. The endpoint may be degraded; retry or switch ` + `providers.`
            });
        }
        // Operators frequently want to see WHEN a chain silently switches
        // pricing modes (e.g. when an RPC starts returning null for
        // `maxFeePerGas` mid-deploy) — without this, a chain that suddenly
        // costs 100× more in legacy mode looks like a black box. We emit a
        // single structured debug line per primitive invocation; production
        // operators typically run with debug-level logs disabled.
        console.debug('[ethers-adapter:calculateFee] EIP-1559 unsupported, falling back to legacy gasPrice', {
            chainId: chain.chainId,
            chainName: chain.name,
...
/**
 * Create an estimate primitive for gas estimation.
 *
 * @param ctx - The Ethers adapter context.
 ...
 *
 * @remarks
 * Calls `provider.estimateGas` to determine gas units, then delegates to
 * `calculateFee` to compute the total transaction cost.
 *
 * @example
 * ```typescript
 * import { createEstimate, createCalculateFee } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const calculateFee = createCalculateFee(ctx)
...
/**
 * Create a simulate primitive for dry-running transactions.
 *
 * @param ctx - The adapter context.
 * @returns A primitive function for simulating transaction execution.
 *
 * @example
 * ```typescript
 * import { createSimulate } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
    ...
    return createPrimitive({
        ...
        execute: async (input, executeCtx)=>{
            ...
            try {
                ...
                // The adapter's error normalizer is throw-based (that's how it
                // feeds the middleware pipeline on every other code path). To
                // keep simulate's soft-failure contract intact — reverts are
                // a returned `success: false` value, not a thrown error — we
                // catch the normalizer's throw locally and attach the resulting
                // `ErrorInfo` to the returned payload so callers can branch on
                // stable codes instead of string-matching `error`.
                let errorInfo;
                try {
                ...
                return {
                    ...
                    // surface — ethers embeds the request URL (and any embedded key)
                    // in error messages, mirroring the redaction applied to
...
/**
 * Translate the shared, viem-shaped EVM transaction params into the field
 * names ethers v6 expects.
 *
 * @remarks
 * {@link buildEVMTxParams} emits `gas` (viem's key). ethers v6
 * `Signer.sendTransaction` reads `gasLimit` and silently ignores `gas`,
 * which would drop any caller-supplied gas override and fall back to
 * auto-estimation. Rename `gas → gasLimit` here so explicit limits are
 * honoured; every other field (fees, nonce, value) maps verbatim.
 *
 * @param params - The viem-shaped params from {@link buildEVMTxParams}.
 * @returns The same params with `gas` renamed to `gasLimit`.
 *
 * @internal
 */ function toEthersTxParams(params) {
...
/**
 * Create an execute primitive with automatic nonce management.
 *
 * @remarks
 * Integrates the legacy {@link EthersNonceManager} to provide:
 * - Automatic nonce allocation when `overrides.nonce` is not set
 * - Nonce error detection and retry (up to 3 attempts)
 * - Atomic resync-and-allocate on nonce conflicts
 *
 * When `overrides.nonce` is explicitly provided, nonce management
 * is bypassed entirely (single attempt, no retry on nonce errors).
 *
 ...
 * broadcasting. After broadcast the transaction is irreversible — aborting
 * the signal only stops the `wait()` promise, never the on-chain transaction.
 *
 * @param ctx - Ethers adapter context.
 * @param waitForTransaction - Wait primitive for confirmation tracking.
 * @returns An execute primitive function.
 */ function createExecute(ctx, waitForTransaction) {
    // Use the per-adapter nonce manager exposed on the context. The
    // module-level `ethersNonceManager` singleton is deprecated because
    // it caused cross-adapter contention when multiple ethers adapters
    // in the same process shared a wallet address — see the deprecation
    // note in `EthersNonceManager.ts` for details.
    const nonceManager = ctx.nonceManager;
    async function executeTransaction(input, chain, traceId, signal) {
        // Cooperative cancellation (A4): refuse to sign/broadcast if already
        // aborted. We never abort between broadcast and confirmation — once
        // `sendTransaction` resolves the tx is live; abort can only stop the
        // `wait()` below, never the on-chain transaction.
        throwIfAborted(signal, 'execute');
        const signer = await ctx.getSigner(chain);
        const provider = await ctx.getProvider(chain);
        const fromAddress = await signer.getAddress();
        const txParams = buildEVMTxParams(input.raw, input.overrides);
        const userProvidedNonce = txParams.nonce !== undefined;
        const maxAttempts = userProvidedNonce ? 1 : MAX_NONCE_RETRY_ATTEMPTS;
        for(let attempt = 1; attempt <= maxAttempts; attempt++){
            try {
                if (!userProvidedNonce) {
                    const nonce = attempt === 1 ? await nonceManager.allocate(provider, chain.chainId, fromAddress) : await nonceManager.resyncAndAllocate(provider, chain.chainId, fromAddress);
                    txParams.nonce = nonce;
                }
                const txResponse = await signer.sendTransaction(toEthersTxParams(txParams));
                return {
                    txId: txResponse.hash,
                    traceId,
                    wait: async ()=>{
                        // Forward the same signal so cancelling the invocation also
                        // stops this post-broadcast wait (the tx keeps settling). Omit
                        // the invocation arg entirely when there is no signal.
                ...
                const canRetry = attempt < maxAttempts && nonceManager.shouldRetryNonceError(error, userProvidedNonce);
                if (!canRetry) {
                    throw error;
                }
            }
        ...
        throw new KitError({
            ...RpcError.ENDPOINT_ERROR,
            recoverability: 'RETRYABLE',
            message: 'Nonce retry loop exhausted without result'
        });
    }
    const primitive = createPrimitive({
        name: 'execute',
        adapterContext: ctx,
        requireAddress: true,
        // See viem execute primitive: when the context doesn't expose an
        // `ensureChain` hook (private-key adapters with `onMismatch: 'skip'`),
        // `createPrimitive` treats this flag as a no-op.
        ensureChainBefore: true,
        // `execute` already handles its own bounded nonce-error retry loop
        // inside `executeTransaction`. The outer retry middleware would add a
        // second layer of replay for any RETRYABLE KitError on submission —
        // a double-spend risk. Keep this primitive single-shot from the
...
/**
 * Create a signTypedData primitive for signing EIP-712 typed data.
 *
 * @param ctx - The adapter context.
 * @returns A primitive function for signing typed data.
 *
 * @example
 * ```typescript
 * import { createSignTypedData } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 ...
 */ function createSignTypedData(ctx) {
    return createPrimitive({
        ...
        execute: async (input, executeCtx)=>{
            ...
            assertTypedData(typedData);
            const signer = await ctx.getSigner(executeCtx.chain);
            const typesWithoutDomain = Object.fromEntries(Object.entries(input.types).filter(([key])=>key !== 'EIP712Domain'));
            // ethers' EIP-712 implementation derives the `EIP712Domain` type
            // descriptor from which fields are actually PRESENT on the
            // `domain` object — `Object.keys(domain).filter((k) => domain[k]
            // != null)`. Previously we materialised every field with a `??
            // null` fallback, which kept the keys present on the object but
            // with `null` values; this caused two bugs:
            //   1. ethers nominally treats `null` as "present", building a
            //      domain descriptor with extra fields (`name`, `version`,
            //      etc.) that the verifier didn't include — every produced
            //      signature then verified against a different domain hash
            //      and got rejected on-chain.
            //   2. Any verifier or relayer that strictly type-checks the
            //      payload (e.g. our own viem path) saw `null` for fields
            //      whose schema is `string` / `number` / hex and rejected
            //      the call before signing even started.
            // Build the domain by COPYING ONLY the fields the caller passed
            // — `undefined` and `null` both mean "absent" here, identical
            // to viem's behaviour. This produces signatures that verify
            // byte-for-byte against the expected domain separator.
            const rawDomain = input.domain;
            const domain = {};
            if (rawDomain.name != null) domain['name'] = rawDomain.name;
            if (rawDomain.version != null) domain['version'] = rawDomain.version;
            if (rawDomain.chainId != null) domain['chainId'] = rawDomain.chainId;
            if (rawDomain.verifyingContract != null) domain['verifyingContract'] = rawDomain.verifyingContract;
            if (rawDomain.salt != null) domain['salt'] = rawDomain.salt;
            const signature = await signer.signTypedData(domain, typesWithoutDomain, input.message);
            return signature;
        }
    });
}
...
/**
 * Create a prepare primitive for EVM transactions.
 *
 * @remarks
 * Accepts both contract call inputs and pre-built raw transactions.
 * For contract inputs the adapter encodes calldata via ethers' `Interface`;
 * for raw inputs the provided {@link RawTransaction} is used as-is.
 *
 * @param options - Configuration containing context and dependent primitives.
 * @returns A prepare primitive function.
 *
 * @example
 * ```typescript
 * import { createPrepare } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const prepare = createPrepare({
 *   ctx: adapterContext,
 *   estimate: createEstimate(ctx, calculateFee),
 *   simulate: createSimulate(ctx),
 *   execute: createExecute(ctx, waitForTransaction),
 * })
 *
 * // Contract call (existing behavior)
 * const result = await prepare(
 *   { address: '0x...', abi: ERC20_ABI, functionName: 'transfer', args: ['0x...', 1000n] },
 *   { chain: Ethereum },
 * )
 *
 * // Raw transaction (new)
 ...
 */ function createPrepare({ ctx: adapterContext, estimate, simulate, execute }) {
    return createPrepare$1({
        ...
        prepare: (input, prepareCtx)=>{
            ...
            const address = prepareCtx.address;
            const operationCtx = {
                chain,
                address
            };
            return inputs.map((item)=>{
                ...
                if (item.type === 'raw') {
                    ...
                    built = {
                        to: item.address,
                        from: address,
                        data: new Interface(item.abi).encodeFunctionData(item.functionName, item.args ?? []),
                        value: item.value ?? 0n
                    };
                }
                return {
 ...
 * `chainId: number | bigint` to match the wider `TypedData` type) to the
 * `SignTypedDataInput.domain` shape used by the Ethers primitive, where
 * `chainId` is explicitly `number | undefined`.
 *
 * @remarks
 * The primitive's Zod schema re-validates the final shape; this narrowing
 * is the single place where the two type systems meet.
 ...
 * @example
 * ```typescript
 * import { createActionsFromAdapter } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 ...
 */ function createActionsFromAdapter(deps) {
    return createEVMActions(deps, {
        getNativeBalance: async (chain, wallet)=>{
            const provider = await deps.ctx.getProvider(chain);
            return provider.getBalance(wallet);
        },
        signTypedData: async (typedData, ctx)=>{
            // Narrow to the EIP-712 shape. The base binding accepts `unknown`
            // because Gateway domains omit chainId / verifyingContract; the
            // ethers primitive re-validates via its own schema before signing.
        ...
        readBytecode: async (address, chain)=>{
            ...
            // ethers' `Provider.getCode` returns `'0x'` for an EOA, never `null`,
            // but normalise just in case a custom provider returns `undefined`.
            return code ?? '0x';
        }
    });
}
 ...
 * @example
 * ```typescript
 * import { createActionDispatch } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
...
// ============================================================================
/**
 * Create a new ethers adapter from options.
 *
 * @remarks
 * Creates the adapter context internally and composes all primitives
 * (read, calculateFee, estimate, simulate, execute, waitForTransaction,
 * prepare, signTypedData) in dependency order, then wraps with legacy
 * compatibility via `withLegacyCompat`.
 *
 * @param options - Adapter options including provider/signer accessors,
 *   capabilities, runtime, and optional token definitions.
 * @returns A fully assembled {@link EthersAdapter} with both new and
 *   legacy APIs.
 * @throws KitError if provider resolution or capability validation fails.
 *
 * @example
 * ```typescript
 * import { createAdapter } from '@circle-fin/adapter-ethers-v6/next'
 * import { JsonRpcProvider, Wallet } from 'ethers'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 ...
 * @returns A fully assembled {@link EthersAdapter}.
 *
 * @example
 * ```typescript
 * import { createAdapterContext, createAdapterFromContext } from '@circle-fin/adapter-ethers-v6/next'
 * import { Ethereum } from '@circle-fin/bridge-kit/chains'
 *
 * const ctx = createAdapterContext({ capabilities, getProvider, getSigner })
...
const getMissingMethodsMessage = (client, requiredMethods, clientType)=>{
    ...
    const methodList = missingMethods.length === 1 ? `core method: ${missingMethods[0] ?? 'unknown'}` : `core methods: ${missingMethods.join(', ')}`;
    return `${clientType} is missing ${methodList}. Please ensure you're using a valid ethers ${clientType}.`;
};
z.object({
    getProvider: z.function().args(z.object({
        chain: z.unknown()
    })).returns(z.unknown()),
    signer: z.unknown().refine((signer)=>signer !== null && signer !== undefined, {
 ...
 */ const createAdapterFromProviderParamsSchema = z.object({
    // Accept either an EIP-1193 provider (`{ request }`) or an ethers
    // `Provider` (`{ getNetwork }`). Server-side users have a fully-
    // fledged ethers Provider, not the EIP-1193 surface — see PR #853
    // round-2 review (finding N8).
    provider: z.object({
        request: z.function().optional(),
        getNetwork: z.function().optional()
    }).passthrough().refine((value)=>typeof value.request === 'function' || typeof value.getNetwork === 'function', {
        message: 'provider must be an EIP-1193 provider ({ request }) or an ethers Provider ({ getNetwork })'
    }),
    getProvider: z.function().args(z.object({
        chain: z.unknown()
    })).returns(z.unknown()).optional(),
 ...
 * ```
 */ z.object({
    address: z.string().min(1, 'Contract address is required. Please provide a valid contract address.'),
    abi: z.any(),
    functionName: z.string().min(1, 'Function name is required. Please provide a valid function name.'),
    args: z.array(z.any(), {
...
// Event types are already declared by the viem adapter's instrumentTransport.ts
// via module augmentation of KitEventMap. No need to redeclare.
const INSTRUMENTED = Symbol.for('@circle-fin/adapter-ethers-v6/instrumented');
/**
 * Patch an ethers provider in place with RPC-level observability.
 *
 * @remarks
 * Patches the provider's `send(method, params)` method in place to intercept
 * JSON-RPC calls. Uses in-place patching (not a Proxy) so that ethers'
 * `instanceof AbstractProvider` checks continue to pass. The caller keeps
 * using the same `base` reference after calling this function.
 *
 ...
 *
 * @param base - The ethers provider to patch (e.g., JsonRpcProvider, BrowserProvider)
 * @param options - Runtime and optional chain label
 */ function instrumentProvider(base, options) {
    const { runtime, chain } = options;
    // ethers Provider does not expose `send()` in its type declarations.
    // We verify it exists at runtime before patching.
    const baseRecord = base;
    if (baseRecord[INSTRUMENTED] === true) {
        return;
    }
    const maybeSend = baseRecord['send'];
    if (typeof maybeSend !== 'function') {
        return;
    }
    const originalSend = maybeSend;
    baseRecord[INSTRUMENTED] = true;
    baseRecord['send'] = async (method, params)=>{
        ...
        try {
            const result = await originalSend.call(base, method, params);
...
/**
 * Create a new Ethers adapter from a private key.
 *
 * @param params - Private key and optional overrides.
 * @returns A configured `EthersAdapter`.
 ...
 */ function createEthersAdapterFromPrivateKey(params) {
    ...
    try {
        ...
        throw new KitError({
            ...InputError.VALIDATION_FAILED,
            recoverability: 'FATAL',
            message: 'Failed to create wallet from private key. Ensure the private key is a valid 32-byte hex string.'
        });
    }
    ...
    const defaultGetProvider = getDefaultProviderUtil(providerCache);
    const wrapProvider = (provider, chainName)=>{
    ...
    const userGetProvider = params.getProvider;
    const getProvider = userGetProvider ? async ({ chain })=>{
        const provider = await Promise.resolve(userGetProvider({
            chainId: chain.chainId,
            chain
        }));
        return wrapProvider(provider, chain.name);
    } : ({ chain })=>wrapProvider(defaultGetProvider({
            chain
        }), chain.name);
    const getSigner = async ({ chain })=>{
        const provider = await Promise.resolve(getProvider({
            chain
        }));
    ...
    return createAdapter({
        ...
        // Private-key adapters have no wallet UI. A chain mismatch here
        // indicates a developer misconfiguration rather than something the
        // adapter can resolve interactively. Default to `'skip'` so server-
        // side flows avoid an extra chain-id round-trip per write; callers
        // wanting a fail-fast check can opt into `'throw'`.
...
/**
 * Heuristic: an ethers `Provider` exposes `getNetwork`. EIP-1193
 * providers do not — they expose `request`. Both `BrowserProvider` and
 * `JsonRpcProvider` satisfy this check, but a plain `window.ethereum`
 * does not.
 *
 * @internal
 */ function isEthersProvider(input) {
    return typeof input === 'object' && input !== null && typeof input.getNetwork === 'function';
}
/**
 * Create a new Ethers adapter from an EIP-1193 provider.
 *
 ...
 * })
 * ```
 */ function createEthersAdapterFromProvider(params) {
    createAdapterFromProviderParamsSchema.parse(params);
    const capabilities = resolveEVMCapabilities(params.capabilities);
    const runtime = resolveRuntime(params.runtime);
    const browserProviderCache = new Map();
    // Detect once: server-side users pass an ethers `Provider`
    // (e.g. `JsonRpcProvider`) which already has its own network and
    // account context. Browser users pass an EIP-1193 provider which we
    // wrap in a per-chain `BrowserProvider`. See
    const ethersProvider = isEthersProvider(params.provider) ? params.provider : undefined;
    // Instrument the ethers Provider so `rpc.call` events fire for every
    // JSON-RPC request. `instrumentProvider` is idempotent (no-ops if
    // already patched or if the provider lacks `send`), so this is safe to
    // call unconditionally. Without this the JsonRpc passthrough path was
    // the only ethers factory entry point with zero RPC telemetry.
    if (ethersProvider !== undefined) {
        instrumentProvider(ethersProvider, {
            runtime
        });
    }
    const wrapProvider = (provider, chainName)=>{
    ...
    const getProvider = ({ chain })=>{
        ...
        let provider = browserProviderCache.get(chain.chainId);
        if (!provider) {
            const raw = new BrowserProvider(params.provider, chain.chainId);
            provider = wrapProvider(raw, chain.name);
            browserProviderCache.set(chain.chainId, provider);
        }
        return provider;
    };
    const getSigner = async ({ chain })=>{
        ...
        try {
            // Both `BrowserProvider` and `JsonRpcApiProvider` (the parent of
            // `JsonRpcProvider`) expose `getSigner()`; for read-only ethers
            // providers (e.g. plain `Provider`) this surface may not exist
            // and the `try`/`catch` below converts the resulting TypeError
            // into a structured `KitError`.
            const providerWithSigner = provider;
            if (typeof providerWithSigner.getSigner !== 'function') {
                throw new TypeError('provider does not expose getSigner() — pass a JsonRpcProvider or BrowserProvider');
            }
            return await providerWithSigner.getSigner();
        } catch (error) {
            // Wrap, but do not interpolate the raw provider message into the
            // user-visible `KitError` message. Provider/wallet errors can carry
            // arbitrary user input (account labels, RPC URLs, payloads) which
            // we do not want to surface verbatim through structured logging —
            // `redactRawError` keeps the post-mortem-useful `{ name, message }`
            // pair on `cause.trace.rawError` while stripping the rest of the
            // provider's object graph.
            throw new KitError({
                ...RpcError.ENDPOINT_ERROR,
                recoverability: 'FATAL',
                message: 'Failed to get signer from provider. Ensure the provider is connected and the user has granted access.',
                cause: {
                    trace: {
                        chain: chain.name,
                        chainId: chain.chainId,
    ...
    // prompting the user on chain mismatch unless they overrode the
    // policy. `developer-controlled` contexts have no wallet UI to
    // prompt, so fall back to `skip` to avoid spurious errors on
    // pinned-chain JSON-RPC providers.
...
export { createActionDispatch, createActionsFromAdapter, createCalculateFee, createEVMActions, createEstimate, createAdapter as createEthersAdapter, createAdapterContext as createEthersAdapterContext, createAdapterFromContext as createEthersAdapterFromContext, createEthersAdapterFromPrivateKey, createEthersAdapterFromProvider, createExecute, createPrepare, createRead, createRuntime, createSignTypedData, createSimulate, createTokenRegistry, createWaitForTransaction, hexAddress, hexBytes32, hexString };
