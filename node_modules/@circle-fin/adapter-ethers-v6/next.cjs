 ...
 */ const prepareInputSchema = zod.z.preprocess((input)=>{
...
const readContractInputSchema = zod.z.object({
    address: hexAddress,
    abi: zod.z.array(zod.z.unknown()).min(1, 'ABI must be a non-empty array'),
    functionName: zod.z.string().min(1, 'Function name is required'),
    args: zod.z.array(zod.z.unknown()).optional()
});
// ============================================================================
            ...
            return inputs.map((item)=>{
                ...
                if (item.type === 'raw') {
                    ...
                    built = {
                        to: item.address,
                        from: address,
                        data: new ethers.Interface(item.abi).encodeFunctionData(item.functionName, item.args ?? []),
                        value: item.value ?? 0n
                    };
                }
                return {
...
zod.z.object({
    getProvider: zod.z.function().args(zod.z.object({
        chain: zod.z.unknown()
    })).returns(zod.z.unknown()),
    signer: zod.z.unknown().refine((signer)=>signer !== null && signer !== undefined, {
 ...
 */ const createAdapterFromProviderParamsSchema = zod.z.object({
    ...
    provider: zod.z.object({
        request: zod.z.function().optional(),
        getNetwork: zod.z.function().optional()
 ...
 * ```
 */ zod.z.object({
    address: zod.z.string().min(1, 'Contract address is required. Please provide a valid contract address.'),
    abi: zod.z.any(),
    functionName: zod.z.string().min(1, 'Function name is required. Please provide a valid function name.'),
 ...
 */ function createEthersAdapterFromProvider(params) {
    ...
    const getProvider = ({ chain })=>{
        ...
        if (!provider) {
            const raw = new ethers.BrowserProvider(params.provider, chain.chainId);
...
exports.createEthersAdapter = createAdapter;
exports.createEthersAdapterContext = createAdapterContext;
exports.createEthersAdapterFromContext = createAdapterFromContext;
